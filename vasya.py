...
def answer_generator(question: str) -> str:
    ...
    data = json.dumps({
        ...
        'messages': [
            {'role': 'system',
                'text': 'Ты находишься в компании единомышленников, '
                'которые угадывают коньяки, бренди '
                'и вина (страна происхождения - Европа и Россия) '
                'по описанию напитка. Ты должен поддержать разговор '
                '(притвориться опытным сомелье) и определить название напитка,'
                ' который был описан словами. И опиши почему ты дал такой '
                'ответ.'},
            {'role': 'user',
                'text': f'{question}'}
        ]
